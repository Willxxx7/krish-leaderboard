the code below is to be put in a file called "server.py"
 and replace the entire server.py with this complete fixed version:
...
To Deploy:
Replace your entire server.py with the fixed version above

Push to GitHub:

bash
git add server.py
git commit -m "Fix IndexError when game scores list is empty"
git push
Wait for Render to deploy (2-3 minutes)

Check your Render site: https://krish-leaderboard.onrender.com

Now the error should be fixed! The webpage will show:

✅ Game scores (with medals if any exist)

✅ Test scores (in green table)

✅ Stats section with safe best time calculation

❌ NO "list index out of range" errors

The test scores from your test_leaderboard.py should now appear in the green table on your Render.com site!
...
------------------------------------------------------------------------
test_leaderboard.py is the filename for code below
